...
fn insert_two_keys_with_same_prefix_and_depth_and_two_more_at_lower_depth_finds_all_four_values() {
	...
	forest.push_trie(index, TestKey(0b000000000100000), 1).expect("push");
	forest.push_trie(index, TestKey(0b000000001000000), 2).expect("push");
	forest.push_trie(index, TestKey(0b000000001000001), 3).expect("push");
	forest.push_trie(index, TestKey(0b000000001000010), 4).expect("push");
...
fn insert_three_keys_with_same_prefix_and_depth_finds_all_three_values() {
	...
	store.push_trie(index, TestKey(0b000000000100000), 1).expect("push");
	store.push_trie(index, TestKey(0b000000001000000), 2).expect("push");
	store.push_trie(index, TestKey(0b000000001100000), 3).expect("push");
...
fn insert_two_keys_on_different_paths_finds_both_values() {
	...
	let index = store.new_trie().expect("index");
	store.push_trie(index, TestKey(0b000000000100000), 1).expect("push");
	store.push_trie(index, TestKey(0b000010000100000), 33).expect("push");
	assert_eq!(2, store[index].size());
	assert_eq!(Some(&1), store[index].find(&TestKey(0b000000000100000)));
...
fn insert_same_key_finds_last_value() {
	...
	let mut store = KvForest::open_or_create(path).expect("open or create");
	let index = store.new_trie().expect("index");
	store.push_trie(index, TestKey(0b000010000000000), 1).expect("push");
	store.push_trie(index, TestKey(0b000010000000000), 2).expect("push");
